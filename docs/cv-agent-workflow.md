...
Write one achievement per line in each entry description. Blank lines are ignored; sentences, acronyms and capitalization are preserved. Aim for 2–3 useful bullets instead of a long abstract. Skills and technologies print as comma-separated text. Dates in `YYYY-MM` form are localized consistently; unsupported date strings remain unchanged. Project status labels are hidden in the date position by default.
